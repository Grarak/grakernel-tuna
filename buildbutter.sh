...
butterversion=v1.3.0
tcf=~/android/kernel/toolchains/

DATE_START=$(date +"%s")

echo -e "${bldcya} Select Toolchain ${txtrst}"
echo -e "  1: Linaro 4.7 13.06"
echo -e "  2: Linaro 4.8 13.06"
read toolchain

if [ "$toolchain" == "1" ]; then
        export CROSS_COMPILE=${tcf}linaro-4.7-13.06/bin/arm-linux-androideabi-
fi

if [ "$toolchain" == "2" ]; then
        export CROSS_COMPILE=${tcf}linaro-4.8-13.06/bin/arm-linux-androideabi-
fi
