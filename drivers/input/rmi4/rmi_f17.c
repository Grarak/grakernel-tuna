...
static int f17_device_init(struct rmi_function_container *fc)
{
	int retval;
...
static int f17_alloc_memory(struct rmi_function_container *fc)
{
	...
	f17 = devm_kzalloc(&fc->dev, sizeof(struct rmi_f17_device_data),
		GFP_KERNEL);
	if (!f17) {
	...
	f17->sticks = devm_kzalloc(&fc->dev, size, GFP_KERNEL);
	if (!f17->sticks) {
...
static int f17_remove_device(struct device *dev)
{
	struct rmi_f17_device_data *f17;
	int i = 0;
...
static int f17_probe(struct device *dev);
...
static __devinit int f17_probe(struct device *dev)
{
