...
static void set_status(struct f05_data *f05, unsigned char new_status)
{
	if (!mutex_is_locked(&f05->status_mutex))
		dev_err(&f05->fn_dev->dev, "ERROR: status_mutex not held for set_status() call!\n");
...
#define WAIT_CONDITION (f05->data_ready || \
	...
	case F05_STATE_PENDING:
		if (count < f05->report_size) {
			dev_err(&fn_dev->dev, "%s: count = %d but need %d bytes -- not enough space\n",
	...
	case F05_STATE_IDLE:
		dev_err(&fn_dev->dev, "ERROR: read() in idle state.\n");
		ret_value = -ECONNRESET;
		goto exit_unlock;
	case F05_STATE_ERROR:
		dev_err(&fn_dev->dev, "ERROR: read() in error state - cleared.\n");
		ret_value = -EIO;
		set_status(f05, F05_STATE_READY);
		goto exit_unlock;
	case F05_STATE_READY:
		dev_err(&fn_dev->dev, "ERROR: read() in ready state.\n");
		ret_value = -EIO;
		goto exit_unlock;
	default:
		dev_err(&fn_dev->dev, "ERROR: F05 is in unrecognized state %d.\n",
			f05->status);
		ret_value = -EIO;
		set_status(f05, F05_STATE_READY);
		goto exit_unlock;
	...
	if (!f05->data_ready) {
		dev_warn(&fn_dev->dev, "WARNING: No data available for read.\n");
		ret_value = -ENODATA;
		goto exit_unlock;
	}
	...
	ret_value = copy_to_user((void __user *)buf,
				 ...
				 f05->report_size);
	if (ret_value) {
		dev_err(&fn_dev->dev, "ERROR: Failed to copy F05 data.\n");
		ret_value = -EFAULT;
		goto exit_unlock;
	}
	ret_value = count;
...
/** This is here because all those casts made for some ugly code.  It doesn't
 * exist in newer driver versions.
 */
static void u8_or(u8 *dest, u8 *target1, u8 *target2, int nbits)
{
	bitmap_or((long unsigned int *) dest,
...
static int f05_char_dev_open(struct inode *inp, struct file *filp)
{
	...
	if (my_dev->busy) {
		dev_warn(&f05->fn_dev->dev, "Device is busy.\n");
		retval = -EBUSY;
	}
	else {
		my_dev->busy = true;
...
static int f05_raw_data_char_dev_register(struct f05_data *f05)
{
	...
	if (!f05) {
		dev_err(&f05->fn_dev->dev, "%s - ERROR: No RMI F05 data structure instance.\n",
			__func__);
		return -EINVAL;
	}
