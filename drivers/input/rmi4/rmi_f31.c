...
struct f31_led_control_0 {
	u8 led_brightness;
};

struct f31_led_control {
	struct f31_led_control_0 *brightness_adjustments;
};

struct f31_data {
	struct f31_led_query led_query;
	struct f31_led_control led_control;
...
static ssize_t rmi_f31_selected_led_brightness_show(struct device *dev,
				struct device_attribute *attr, char *buf);

static ssize_t rmi_f31_selected_led_brightness_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count);
...
static int rmi_f31_reset(struct rmi_function_container *fc);
...
static struct device_attribute brightness_attrs[] = {
	...
	__ATTR(selected_led_brightness, RMI_RW_ATTR,
		   rmi_f31_selected_led_brightness_show,
		   rmi_f31_selected_led_brightness_store),
};
...
static int rmi_f31_alloc_memory(struct rmi_function_container *fc)
{
	...
	if (f31->led_query.has_brightness) {
		f31->led_control.brightness_adjustments =
			kcalloc(f31->led_count,
				sizeof(struct f31_led_control_0), GFP_KERNEL);
		if (!f31->led_control.brightness_adjustments) {
			dev_err(&fc->dev, "Failed to allocate brightness_adjustments.\n");
			return -ENOMEM;
		}
	}
...
static void rmi_f31_free_memory(struct rmi_function_container *fc)
{
	...
	if (f31) {
		kfree(f31->led_control.brightness_adjustments);
		kfree(f31);
		fc->data = NULL;
	}
}
	...
	if (f31->led_query.has_brightness) {
		for (i = 0; i < f31->led_count; i++) {
			rc = rmi_read_block(rmi_dev, read_addr,
				&(f31->led_control.brightness_adjustments[i].led_brightness),
				sizeof(struct f31_led_control_0));
			if (rc < 0) {
			...
			read_addr = read_addr + sizeof(struct f31_led_control_0);
		}
	}
...
static int rmi_f31_config(struct rmi_function_container *fc)
{
	...
	if (f31->led_query.has_brightness) {
		for (i = 0; i < f31->led_count; i++) {
			...
			write_addr = write_addr + sizeof(struct f31_led_control_0);
		}
	}
...
static int f31_probe(struct device *dev);
					...
					const char *buf, size_t count)
{
	...
	if (new_value < 0 || new_value > data->led_count - 1) {
				...
				struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%u\n",
		data->led_control.brightness_adjustments[data->selected_led].led_brightness);
}
					...
					const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &new_value) != 1) {
		dev_err(dev, "Selected_led_brightness_store has an invalid len.\n");
		return -EINVAL;
	}
	...
	data->led_control.brightness_adjustments[data->selected_led].led_brightness = new_value;

	write_addr = fc->fd.control_base_addr + (data->selected_led * sizeof(struct f31_led_control_0));
