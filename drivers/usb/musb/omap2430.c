...
BLOCKING_NOTIFIER_HEAD(musb_init_notifier);

int musb_register_init_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&musb_init_notifier, nb);
}
EXPORT_SYMBOL_GPL(musb_register_init_notifier);

static void musb_signal_init(void)
{
	blocking_notifier_call_chain(&musb_init_notifier, 0, NULL);
}
...
int omap_musb_mailbox(enum omap_musb_vbus_id_status status)
{
	...
	if (!musb) {
		printk("%s: musb core is not yet ready\n", __func__);
		return -ENODEV;
	}
...
static void omap_musb_set_mailbox(struct omap2430_glue *glue)
{
	...
	dev_dbg(dev, "%s: last_event=%d gadget_driver=%p control_dev=%p "
			"is_otg_enabled=%d is_peripheral_active=%d\n", __func__,
